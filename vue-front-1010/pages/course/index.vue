          ...
          <dl>
            ...
            <dd class="c-s-dl-li">
              ...
              <ul class="clearfix">
                ...
                <li v-for="(item,index) in subSubjectList" :key="item.id">
                ...
                <div class="cc-l-wrap">
                  ...
                  <section class="mt10 hLh20 of">
                    ...
                    <span class="fr jgTag bg-green">
                      ...
                      <i class="c-fff fsize12 f-fA">{{Number(item.price)===0?'免费':'付费' }}</i>
...
export default {
  data() {
    return {
      ...
      oneIndex: -1, //一级分类选中标识,为了样式生效
      twoIndex: -1, //二级分类选中标识,为了样式生效
      buyCountSort: '', //销量排序选中标识,为了样式生效
      gmtCreateSort: '', //最新时间排序选中标识,为了样式生效
      priceSort: '', //价格排序选中标识,为了样式生效
    }
  },
  created() {
    if (this.$route.query.title) {
      this.searchObj.title = this.$route.query.title //根据title模糊查询
      //查询第一页数据
      this.courseFind(1)
    } else {
      //查询第一页数据
  ...
  methods: {
    ...
    searchOne(subjectParentId, index) {
      ...
      this.subSubjectList = this.subjectNestedList
        .filter((item) => item.id === subjectParentId)
